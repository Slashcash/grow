...
RUN groupadd --gid $(id -g) grow && \\
    useradd --no-log-init -rm -s /bin/bash -u $(id -u) -g $(id -g) grow
    
# Download, compile and install source code
RUN cd / && \\
    git clone https://github.com/Slashcash/grow.git && \\
    cd grow && \\
    mkdir build && \\
    cd build && \\
    cmake .. -DGROW_BUILD_TESTS=OFF -DGROW_BUILD_EXAMPLES=OFF && \\
    cmake --build . --target all && \\
    cmake --install ." > "${OUTPUT_PATH}"

if [ -f "$OUTPUT_PATH" ]; then
  docker build -t grow_simulation:v${PROJECT_VERSION} -f ${OUTPUT_PATH} ${CONTEXT_DIR}
else
